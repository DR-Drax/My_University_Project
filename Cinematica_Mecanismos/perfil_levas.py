...
def interfaz_pedir_datos():
    ...
    def editar_dato(matriz, tipos):
        ...
        while True:
            ...
            etapa = check_num_input(0, len(matriz[0]), "Número de etapa: ")
            if etapa == 0:
                break

            etapa -= 1  # Ajustar índice
            print(f"\nEditando etapa {etapa + 1}:")
            matriz[0][etapa] = check_float_input("Duración de la etapa (s): ")
            matriz[1][etapa] = check_num_input(1, 3, "Tipo de movimiento (1: Ascenso, 2: Plano, 3: Descenso): ")
            
            if matriz[1][etapa] == 2:
                matriz[2][etapa] = 0
                tipos[etapa] = 1
            else:
                matriz[2][etapa] = check_float_input("Altura asociada (mm): ")
                tipos[etapa] = check_num_input(1, 3, "Tipo de curva (1: Lineal, 2: Armónica, 3: Cicloidal): ")

    clear_screen()
    print("\n=== Parámetros de la leva ===")
    
    # Número de etapas
    num_etapas = check_float_input("Ingrese el número de etapas: ")
    ...
    for i in range(int(num_etapas)):
        ...
        # Tiempo de la etapa
        matriz_datos[0].append(check_float_input("\tDuración de la etapa (s): "))
        
        # Tipo de movimiento (Ascenso, Plano, Descenso)
        movimiento = check_num_input(1, 3, "\tTipo de movimiento (1: Ascenso, 2: Plano, 3: Descenso): ")
        ...
        if movimiento == 2:
            ...
            # Altura asociada
            matriz_datos[2].append(check_float_input("\tAltura asociada (mm): "))
            # Tipo de curva para cada etapa
            tipo_movimiento.append(check_num_input(1, 3, "\tTipo de curva (1: Lineal, 2: Armónica, 3: Cicloidal): "))
...
while flag == True:
    ...
    y += 1
    a = check_num_input(1,2,"Quiere hacer otra grafica? (1: Si  2: No)")
...
#y = levas_table([[3,1,6,2],[1,2,3,2],[10,10,0,0]],[2,1,3,1],100,y)

#EJE 2 
#y = levas_table([[.5,.1,.6],[1,2,3],[.5,.5,0]],[2,2,2],100,y)

#EJE 3 ---- 
#y = levas_table([[4,1,4,1,2],[1,2,3,2,3],[1,1,.5,.5,0]],[3,3,3,3,3],100,y)

#EJE 4 ---- 
#y = levas_table([[.8,.3,.6,.3,.7],[1,2,3,2,3],[1,1,.5,.5,0]],[3,3,3,3,3],100,y)
